...
static inline void WritePicaReg(u32 id, u32 value, u32 mask) {
    ...
    switch(id) {
        ...
        case PICA_REG_INDEX(trigger_draw_indexed):
        {
            ...
            const u8* index_address_8 = Memory::GetPointer(PAddrToVAddr(attribute_config.base_address * 8 + index_info.offset));
                ...
                for (int i = 0; i < attribute_config.GetNumTotalAttributes(); ++i) {
                    for (unsigned int comp = 0; comp < vertex_attribute_elements[i]; ++comp) {
                        ...
                        DEBUG_LOG(GPU, "Loaded component %x of attribute %x for vertex %x (index %x) from 0x%08x + 0x%08lx + 0x%04lx: %f",
                                  comp, i, vertex, index,
                                  attribute_config.base_address * 8,
        ...
        case PICA_REG_INDEX(vs_bool_uniforms):
            for (unsigned i = 0; i < 16; ++i) {
                ...
                ERROR_LOG(GPU, "Set boolean uniform %d to %s", i, VertexShader::GetBoolUniform(i) ? "true" : "false");
            }
