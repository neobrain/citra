        ...
        case 2:
        {
            ...
            if (instr.opcode.GetInfo().type == Instruction::OpCodeType::Arithmetic) {
                ...
                output << std::setw(4) << std::right << instr.common.dest.GetName() << "." << swizzle.DestMaskToString() << "  "
                    << std::setw(8) << std::right << ((swizzle.negate_src1 ? "-" : "") + instr.common.src1.Value().GetName()) + src1_relative_address << "." << swizzle.SelectorToString(false) << "  ";

                if (instr.opcode.GetInfo().subtype & Instruction::OpCodeInfo::Src2)
                    output << std::setw(4) << std::right << (swizzle.negate_src2 ? "-" : "") + instr.common.src2.Value().GetName() << "." << swizzle.SelectorToString(true) << "   ";
